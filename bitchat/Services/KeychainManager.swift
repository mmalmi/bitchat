...
final class KeychainManager: KeychainManagerProtocol {
    ...
    #if os(macOS)
    /// Fail closed instead of letting a background keychain read summon a
    /// login-keychain prompt repeatedly.
    private func addNonInteractiveAuthenticationContext(
    ...
    private func retrieveDataWithResult(forKey key: String) -> KeychainReadResult {
        ...
        func attempt(withAccessGroup: Bool) -> OSStatus {
            ...
            #if os(macOS)
            addNonInteractiveAuthenticationContext(to: &q)
            #endif
    ...
    func save(key: String, data: Data, service customService: String, accessible: CFString?) {
        ...
        var deleteQuery = primaryKeyQuery
        #if os(macOS)
